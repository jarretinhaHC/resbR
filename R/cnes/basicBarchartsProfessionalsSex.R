...
### Focus on professionals and their sex
...
for(cir in unique(selected_regions$CO_CIR)){
    ...
    tmp <- fdata[, c('CO_CNES', 'NU_COMPETENCIA', 'CO_CPF', 'SEXO'), with=F]
    counts <- tmp[, {v <- charter(CO_CNES, CO_CPF);
                  list(VÍNCULOS=as.integer(names(v)),
                       CONTAGEM=as.vector(v))},
                  by=c('NU_COMPETENCIA', 'SEXO')]

    counts <- counts[, SOMA := sum(CONTAGEM), by=c('NU_COMPETENCIA', 'SEXO')]
    counts <- counts[, PERCENTUAL := sprintf('%2.3f', CONTAGEM/SOMA * 100)]
    counts <- counts[, SOMA := NULL]

    wb_name <- paste0(region_name, ' - Seção 01 - Vínculos por profissional e sexo.xls')
    ...
    wb_name <- paste0(region_name, ' - Seção 01 - Estabelecimentos por profissional e sexo.xls')
    wb <- loadWorkbook(paste(focalDir, wb_name, sep='/'), create=TRUE)
    counts <- split(counts, counts$NU_COMPETENCIA)
    ...
    tmp <- fdata[, c('NU_COMPETENCIA', 'CO_CPF', 'MUN.NO_MUNICIPIO', 'SEXO'), with=F]
    tmp <- unique(tmp)
    ...
    wb_name <- paste0(region_name, ' - Seção 01 - Municípios por profissional e sexo.xls')
    wb <- loadWorkbook(paste(focalDir, wb_name, sep='/'), create=TRUE)
    counts <- split(counts, counts$NU_COMPETENCIA)
    ...
    tmp <- fdata[, c('NU_COMPETENCIA', 'CO_CPF', 'DS_CBO_OCUPACAO', 'SEXO'), with=F]
    tmp <- unique(tmp)
    ...
    wb_name <- paste0(region_name, ' - Seção 01 - Ocupações por profissional e sexo.xls')
    wb <- loadWorkbook(paste(focalDir, wb_name, sep='/'), create=TRUE)
    counts <- split(counts, counts$NU_COMPETENCIA)
