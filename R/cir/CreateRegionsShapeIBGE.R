...
regions_stats <- read.table(file.path(paste(refDir, 'Mobilidade por região e município - Brasil.tsv', sep='/')),
                            sep='\t',
                            header=TRUE,
                            stringsAsFactor=FALSE,
                            as.is=TRUE,
...
# Change CD_GEOCODM to the meaningful CO_IBGE
names(shp)[2] <- 'CO_IBGE'
...
shp@data <- merge(shp@data, regions, by='CO_IBGE')

# Merge cities into regions
tmp <- gUnionCascaded(shp, id=shp@data$CO_CIR)
tmp <- SpatialPolygonsDataFrame(tmp, data.frame(CO_CIR=row.names(tmp), row.names=row.names(tmp)))
